...
int main(void){
    ...
    for(Temp = 0.0001;Temp<0.01;Temp*=10.){
        char filename[128];
        sprintf(filename, "./output/test/temp_%.4f", Temp);
        ...
        for(i=0;i<N;i++){//this loop is ensemble
            init_array(x,v); //set initial conditions for every particle
            bool check = true;
